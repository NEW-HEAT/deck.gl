...
const LegacyPriorityTileset2D = class extends (Tileset2D as any) {
  _getRequestPriority(tile): number {
    if (tile.isSelected) {
      return 0;
    }
    if (tile.isVisible) {
  ...
  const layer = useMemo(
    () =>
      new TileLayer<DemoTile>({
        id: `${mode}-tiles`,
        TilesetClass: mode === 'before' ? LegacyPriorityTileset2D : Tileset2D,
        minZoom: 0,
        maxZoom: 20,
        tileSize: 512,
  ...
  return (
    <div style={styles.stage}>
      <DemoPane
        ...
        title="Before"
        subtitle="Equal request priority"
      />
      <DemoPane
        mode="after"
        viewState={viewState}
        generation={generation}
        title="After"
        subtitle="Coverage LOD + center priority"
      />
