...
test('GlobeViewport#isPointOnGlobe', () => {
  ...
  expect(
    viewport.getGlobeRayDistanceRatio([viewport.width / 2, viewport.height / 2]),
    'screen center ray is more stable than edge ray'
  ).toBeLessThan(viewport.getGlobeRayDistanceRatio([viewport.width, viewport.height / 2]));
});
