...
test('TerrainLayer#caps stitched texture fanout', async () => {
  const urls: string[] = [];
  const layer = new TerrainLayer({
    id: 'terrain',
    elevationData: 'terrain/{z}/{x}/{y}.png',
  ...
  expect(urls.slice(1), 'limits texture stitching to a bounded child tile fanout').toHaveLength(16);
  expect(urls[1], 'first texture child reaches capped texture zoom').toBe('texture/15/4/8.png');
});
