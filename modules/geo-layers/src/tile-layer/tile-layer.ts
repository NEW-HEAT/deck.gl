...
type _TileLayerProps<DataT> = {
  ...
  /**
   * Renders temporary placeholder layers while a visible tile is still loading.
   * Placeholder layers are not cached on the tile, so loaded content replaces
  ...
  renderLayers(): Layer | null | LayersList {
    ...
    return this.state.tileset!.tiles.map((tile: Tile2DHeader) => {
      ...
      if (!tile.isLoaded && !tile.content) {
        ...
        return (flatten(layers, Boolean) as Layer<{tile?: Tile2DHeader}>[]).map(layer => {
