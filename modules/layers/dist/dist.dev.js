...
`
  ...
  function convertColor(colorDef = {}) {
    const { color: color5 = [0, 0, 0], intensity = 1 } = colorDef;
    return color5.map((component) => component * intensity / COLOR_FACTOR);
  }
...
`
  ...
  function tesselateColumn(props) {
    ...
    const numVertices = !isExtruded ? nradial : bevelSegments >= 2 ? vertsAroundEdge * 2 + 2 + bevelLevels * vertsAroundEdge * 2 + 1 : vertsAroundEdge * 3 + 1;
    ...
    const getXY = (j, scale = 1) => {
      const vIdx = j % nradial;
      return vertices ? [vertices[vIdx * 2] * scale, vertices[vIdx * 2 + 1] * scale] : [Math.cos(j * stepAngle) * radius * scale, Math.sin(j * stepAngle) * radius * scale];
    };
    const getNormal = (j) => {
      const vIdx = j % nradial;
      return vertices ? [vertices[vIdx * 2], vertices[vIdx * 2 + 1]] : [Math.cos(j * stepAngle), Math.sin(j * stepAngle)];
    };
    if (isExtruded) {
    ...
    if (isExtruded && bevelSegments >= 2) {
      ...
      for (let level = 0; level < bevelLevels; level++) {
        ...
        for (let j = 0; j < vertsAroundEdge; j++) {
          ...
          if (smoothNormals) {
            setVertex([x1, y1, z1], [cos2 * Math.cos(phi1), sin2 * Math.cos(phi1), Math.sin(phi1)]);
            setVertex([x2, y2, z2], [cos2 * Math.cos(phi2), sin2 * Math.cos(phi2), Math.sin(phi2)]);
          } else {
            setVertex([x1, y1, z1], [cos2 * dz / len, sin2 * dz / len, -dr / len]);
            setVertex([x2, y2, z2], [cos2 * dz / len, sin2 * dz / len, -dr / len]);
          }
        }
      }
...
`;
  ...
  function bevelPropToHeight(bevel) {
    if (bevel === "flat")
      return 0;
    if (bevel === "dome")
      return -1;
  ...
  function bevelPropToSegs(bevel) {
    if (bevel === "flat")
      return 0;
    if (bevel === "dome")
      return -1;
  ...
  function bevelPropToBulge(bevel) {
    if (typeof bevel === "object" && bevel !== null) {
  ...
  var ColumnLayer = class extends import_core13.Layer {
    ...
    updateState(params) {
      ...
      if (regenerateModels || props.diskResolution !== oldProps.diskResolution || props.bevelSegments !== oldProps.bevelSegments || props.vertices !== oldProps.vertices || (props.extruded || props.stroked) !== (oldProps.extruded || oldProps.stroked)) {
        this._updateGeometry(props);
      }
    }
    getGeometry(diskResolution, vertices, hasThickness) {
