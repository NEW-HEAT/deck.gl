...
function tesselateColumn(props) {
  const { radius, height = 1, nradial = 10, bevelSegments = 0, bevelHeight = 1, smoothNormals = false } = props;
  let { vertices } = props;
  if (vertices) {
  ...
  if (isExtruded) {
    ...
    setVertex([positions[i - 3], positions[i - 2], positions[i - 1]], [normals[i - 3], normals[i - 2], normals[i - 1]]);
  }
